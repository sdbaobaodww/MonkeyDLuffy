...
/**
 注册IOCBean，有3种使用类型：
 1，Bean组。protocol继承MDLIOCBundle协议，表示该协议使用Bean组，Bean组指为协议绑定多个实现对象的情形，Bean组注册时会生成一个协议代理对象MDLProtocolProxy，Bean组的获取会返回该协议代理对象，使用时对代理对象进行调用方法，可视为对所有绑定对象调用方法
 2，Bean有别名。一个协议在不同业务使用不同的实现类，使用别名可进行区分，获取时，使用不同别名，可获取各业务的实现类。
 3，通常情况下协议只绑定一个实现类
 
 @param bean IOC注入描述对象
 @param key Bean或Bean组关键字
 */
...
/**
 Bean组取消注册方法，clazz为Nil时，表示取消注册Bean组，clazz不为Nil时，表示取消注册Bean组中指定clazz实现
 @param beanKey Bean组关键字
 @param clazz 指定类，Bean组时用到，如果有指定，则移除Bean组绑定clazz的bean，否则移除整个Bean组
 */
...
/**
 ...
 二、Bean组，protocol继承MDLIOCBundle协议，则会返回协议代理对象（MDLProtocolProxy），对代理对象的方法调用，视为对Bean组所有对象的调用；
 三、非Bean组：
 1，使用缓存策略的第一次调用时创建，创建后只要上下文未释放就会一直存在；
 2，不使用缓存策略的每次调用时都会创建；
 
 @param beanKey Bean或Bean组关键字
 @param beanBundle 是否是Bean组
 @return 生成的实例
 */
